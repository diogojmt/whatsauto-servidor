...
class TestRunner {
  ...
  async testCpfValidacao() {
    ...
    if (
      !resp.reply.includes("11 dígitos") &&
      !resp.reply.includes("14 dígitos")
    ) {
  ...
  async testFluxoCompleto() {
    ...
    try {
      ...
      if (respCpf.reply.includes("Certidão emitida")) {
      ...
      } else if (respCpf.reply.includes("Cadastro Geral")) {
        // Continuar com inscrição manual
        const respInsc = await this.sendMessage(user, "113436");

        if (
          respInsc.reply.includes("Certidão emitida") ||
          respInsc.reply.includes("https://")
        ) {
        ...
        throw new Error(
          `Resposta inesperada: ${respCpf.reply.substring(0, 100)}...`
        );
      }
    } catch (error) {
  ...
  printReport() {
    ...
    console.log(
      `📈 Taxa de sucesso: ${((passed / this.results.length) * 100).toFixed(
        1
      )}%`
    );
