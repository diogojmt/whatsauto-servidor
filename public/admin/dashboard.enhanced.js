...
class DashboardApp {
    ...
    async loadSectionData(section) {
        const sectionLoaders = {
            ...
            usuarios: () => {/* Implementação futura */},
            relatorios: () => {/* Não requer carregamento */}
        };
    ...
    createCharts(data) {
        const chartCreators = {
            tiposPopulares: () => this.createTiposPopularesChart(data.tiposAtendimento),
            atendimentosHora: () => this.createAtendimentosHoraChart(data.atendimentosPorHora),
            atendimentosDia: () => this.createAtendimentosDiaChart(data.atendimentosPorDia),
    ...
    async exportarRelatorio(tipo) {
        try {
            const response = await fetch(`${this.state.apiBase}/export?tipo=${tipo}`, {
                headers: {
                    'Authorization': `Bearer ${this.state.token}`
                }
            });
