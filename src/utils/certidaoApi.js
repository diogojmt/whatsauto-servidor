...
async function emitirCertidao({ tipoContribuinte, inscricao, operacao = "2" }) {
  ...
  try {
    ...
    return { 
      SSACodigo: 99, 
      SSAMensagem: 'Erro na comunicação com o serviço de certidões. Tente novamente em alguns minutos.' 
    };
  }
}
 ...
 * @param {string} inscricao - Inscrição municipal
...
function validarDadosCertidao(tipoContribuinte, inscricao) {
  ...
  if (!tipoContribuinte || !['1', '2', '3'].includes(tipoContribuinte)) {
    errors.push('Tipo de contribuinte deve ser 1 (PF/PJ), 2 (Imóvel) ou 3 (Empresa)');
  }
  ...
  if (inscricao && !/^\d+$/.test(inscricao.trim())) {
    errors.push('Inscrição municipal deve conter apenas números');
  }
